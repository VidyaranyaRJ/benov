...
AWS_REGION="us-east-2"
TERRAFORM_DIR="$(dirname "$0")"
# Auto-discover instance ID - choose one of these methods:

# Method 1: Get the first running instance in the region
...
# Method 2: Get instance by tag (uncomment and modify if you use tags)
# INSTANCE_ID=$(aws ec2 describe-instances \
#     --region "$AWS_REGION" \
#     --filters "Name=tag:Name,Values=YourInstanceName" "Name=instance-state-name,Values=running" \
#     --query 'Reservations[0].Instances[0].InstanceId' \
#     --output text)

# Method 3: Get from Terraform output (if you manage instances with Terraform)
# INSTANCE_ID=$(terraform -chdir="$TERRAFORM_DIR" output -raw instance_id 2>/dev/null || echo "")

# Method 4: Interactive selection (uncomment to use)
...
# Validate instance ID was found
if [ -z "$INSTANCE_ID" ] || [ "$INSTANCE_ID" = "None" ] || [ "$INSTANCE_ID" = "null" ]; then
    echo "ERROR: Could not automatically discover instance ID!"
    echo "Available running instances in $AWS_REGION:"
    aws ec2 describe-instances --region "$AWS_REGION" \
        --filters "Name=instance-state-name,Values=running" \
        --query 'Reservations[*].Instances[*].{InstanceId:InstanceId,Name:Tags[?Key==`Name`].Value|[0],Type:InstanceType,State:State.Name}' \
        --output table
    exit 1
fi
